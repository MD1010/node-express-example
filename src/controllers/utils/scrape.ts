...
export const scrapData = async () => {
  try {
    ...
    let allMusclesGroupPage = await axios.get(`${MAIN_SCRAP_URL}/exercise/muscle/`);
    const dom = new JSDOM(allMusclesGroupPage.data);

    let musclesGroups = dom.window.document.querySelectorAll("body > div > div > div > div > ul > li > a");
    ...
    await Promise.all(
      Array.from(musclesGroups)
        ...
        .map(async (muscleGroup) => {
          let muscleGroupUrl = `${MAIN_SCRAP_URL}${muscleGroup.getAttribute("href")}`;
          ...
          const createdMuscleGroupId = await generateMusclesGroup(createdMusclesIds, muscleGroupPageDom);
          let exercises = muscleGroupPageDom.window.document.querySelectorAll(".panel>h5>a");
          await Promise.all(
...
export const generateMuscles = async (dom: JSDOM) => {
  ...
  let muscles = dom.window.document.querySelectorAll("body > div > div > div > div.columns.large-8 > ul > li> a");
  await Promise.all(
...
async function generateMusclesGroup(createdMusclesIds: any[], dom: JSDOM) {
  ...
  let createdMuscleGroup = await MuscleGroupEntity.create(finalMuscleGroupObjectToDb as any);
  console.log("Created muscleGroup");
  return createdMuscleGroup._id;
}

function scrapeMusclesEntitiesDetails(dom: JSDOM) {
  let name = dom.window.document.querySelector(".columns>h1")?.textContent?.trim();
  let description = dom.window.document.querySelector(".columns > p")?.textContent?.trim();
  let image = `${MAIN_SCRAP_URL}${dom.window.document.querySelector("#body_map-15~img")?.getAttribute("src")}`;
...
async function scrapExercise(exerciseUrl: string) {
  ...
  let primaryMusclesSection = dom.window.document.querySelectorAll("tbody>tr>td")[0].children;
  let secondaryMusclesSection = dom.window.document.querySelectorAll("tbody>tr>td")[1].children;
...
async function generateExercise(exerciseObject: any, createdMuscleGroupId: string) {
  let primaryIds: any[] = [];
